...
| AI 工作時間表 | 日曆式的日／週／月，可往前往後翻。上方全天列是排定的任務，時間軸是實際跑過的紀錄 |
...
兩頁吃同一個 `projects` API，畫法不同。**看板總覽**只管任務：排在哪一天、做完了沒，週七格、月曆格。
**AI 工作時間表**多一層實際執行的時間軸，看得出 Claude Code 與 Cowork 真的跑了什麼、跑了多久。
